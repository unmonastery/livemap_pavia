	...
	$.ajax({
	  url: '/data/stops',
		  success: function(data) {
			  ...
			  L.geoJson(data, {
					pointToLayer: function(f, latlng) { return new L.Marker(latlng, {icon : hIcon }).bindPopup('<b>'+f.properties.stop_name+'</b><br>'); }
			  }).addTo(map);  
		  }
	});	
		...
		for(var trip in data){
			if(data.hasOwnProperty(trip)){
				...
				if(!knownTrips[trip]){
					...
					if(trips && trips[trip]){
						popup = "<b>Bus: "+trips[trip].route_short_name+"</b><br>"+trips[trip].route_long_name+"<br><i>"+trip+"</i>";
						//bus or tram?
